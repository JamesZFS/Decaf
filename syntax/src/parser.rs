...
impl<'p> Parser<'p> {
    ...
    fn stmt_for(f: Token, _l: Token, init: Stmt<'p>, _s1: Token, cond: Expr<'p>, _s2: Token,
                update: Stmt<'p>, _r: Token, body: Stmt<'p>) -> Stmt<'p> {
    ...
    fn expr_lambda0(f: Token, _l: Token, params: Vec<&'p VarDef<'p>>, _r: Token, _a: Token, e: Expr<'p>) -> Expr<'p> {
        mk_expr(f.loc(), Lambda { loc: f.loc(), name: format!("lambda@{:?}", f.loc()), params, ret_ty: dft(), can_tys: dft(), body: LambdaKind::Expr(Box::new(e), dft()), ret_param_ty: dft(), class: dft(), scope: dft(), cap_list: dft(), cap_this: Cell::new(false) }.into())
    }
    ...
    fn expr_lambda1(f: Token, _l: Token, params: Vec<&'p VarDef<'p>>, _r: Token, b: Block<'p>) -> Expr<'p> {
        mk_expr(f.loc(), Lambda { loc: f.loc(), name: format!("lambda@{:?}", f.loc()), params, ret_ty: dft(), can_tys: dft(), body: LambdaKind::Block(Box::new(b)), ret_param_ty: dft(), class: dft(), scope: dft(), cap_list: dft(), cap_this: Cell::new(false) }.into())
    }
    ...
    fn var_sel(owner: Option<Box<Expr<'p>>>, name: Token) -> Expr<'p> {
        mk_expr(name.loc(), VarSel { owner, name: name.str(), field: dft() }.into())
    }
