...
impl<'p> Parser<'p> {
    ...
    fn expr_lambda0(f: Token, _l: Token, params: Vec<&'p VarDef<'p>>, _r: Token, _a: Token, e: Expr<'p>) -> Expr<'p> {
        mk_expr(f.loc(), Lambda { loc: f.loc(), name: format!("lambda@{:?}", f.loc()), params, body: LambdaKind::Expr(Box::new(e), dft()), ret_param_ty: dft(), class: dft(), scope: dft() }.into())
    }
    ...
    fn expr_lambda1(f: Token, _l: Token, params: Vec<&'p VarDef<'p>>, _r: Token, b: Block<'p>) -> Expr<'p> {
        mk_expr(f.loc(), Lambda { loc: f.loc(), name: format!("lambda@{:?}", f.loc()), params, body: LambdaKind::Block(Box::new(b)), ret_param_ty: dft(), class: dft(), scope: dft() }.into())
    }
