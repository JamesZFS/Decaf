...
impl<'a> BB<'a> {
  ...
  pub fn next_with_entry<'b>(&'b self, this: usize) -> (usize, impl IntoIterator<Item=usize> + Clone + 'b) {
    (this, match self.next {
      NextKind::Ret(_) => vec![].into_iter(),
      NextKind::Jmp(trg) => vec![trg as usize].into_iter(),
      NextKind::Jif { fail, jump, .. } => vec![fail as usize, jump as usize].into_iter(),
      NextKind::Halt => vec![].into_iter(),
    })
  }
}
